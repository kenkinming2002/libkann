...
namespace tensor
{
  ...
  std::vector<Tensor<T>> split_outer(Tensor<T> value)
  {
    ...
    const auto& sub_shape = value.shape.drop_front(1);
    ...
    for(size_t i=0; i<M; ++i)
      results.push_back(Tensor<T>(sub_shape, std::move(results_buffer[i])));
    return results;
  }
  ...
  {
    const auto& shape = value.shape;
    const auto& sub_shape = value.shape.drop_back(1);
