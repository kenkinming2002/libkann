...
namespace cereal
{
  ...
  template<typename Archive, typename Scalar, int Rows, int Cols, int Options, int MaxRows, int MaxCols>
  void save(Archive& archive, const Eigen::Array<Scalar, Rows, Cols, Options, MaxRows, MaxCols>& matrix)
  {
    archive(make_size_tag(matrix.size()), matrix.rows(), matrix.cols());
  ...
  template<typename Archive, typename Scalar, int Rows, int Cols, int Options, int MaxRows, int MaxCols>
  void load(Archive& archive, Eigen::Array<Scalar, Rows, Cols, Options, MaxRows, MaxCols>& matrix)
  {
    size_type size;
    Eigen::Index rows, cols;
    archive(make_size_tag(size), rows, cols);
